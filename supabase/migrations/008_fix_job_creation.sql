-- Migration: 008_fix_job_creation.sql
-- Description: Fixes create_job_from_form to include lat/lng, set status to open, and link customer properly
...
DROP FUNCTION IF EXISTS public.create_job_from_form(text, text, text, text, text, text, text, text, text, text, text, text, text, text, text, text, text, timestamp with time zone, timestamp with time zone, numeric, uuid);
...
CREATE OR REPLACE FUNCTION public.create_job_from_form(
    ...
    p_job_latitude NUMERIC DEFAULT NULL,
    p_job_longitude NUMERIC DEFAULT NULL
) RETURNS jsonb AS $$
DECLARE
    v_customer_id UUID;
    v_address_id UUID;
...
BEGIN
    ...
    -- 3. Create Job
    -- Fix: Added latitude, longitude and changed default status to 'open'
    ...
    VALUES (
        ...
        'open', p_job_latitude, p_job_longitude
    )
